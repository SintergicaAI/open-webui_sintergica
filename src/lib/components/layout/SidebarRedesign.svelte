<script lang="ts">
	...
	const routes = [
		...
		{ name: 'Canales', path: '/dev/channels', icon: MessagesSquare },
		{ name: 'Usuarios', path: '/dev/admin/users', icon: UserPen },
		{ name: 'Usuarios', path: '/dev/admin/groups', icon: Users },
		{ name: 'Conocimiento', path: '/dev/knowledge', icon: LibraryBig },
		{ name: 'Asistentes', path: '/dev/models', icon: TuringFace}
	];
</script>
...
<aside class="sidebar {customClass}">
	...
	<div class="button-group">
			<Button variant="icon" size="lg" iconSize="lg" buttonClasses=" text-slate-500"
							onClick={async () => {
								await userSignOut();
...
<style lang="scss">
  ...
  .sidebar {
    ...
    @apply py-sm space-y-base dark:bg-slate-800;
  }

...
<!--	</DropdownMenu.Trigger>-->
	...
	>
		<div class="flex gap-x-base items-center">
			<div class="flex flex-col justify-center align-center items-center rounded-full bg-brand-100 w-8 h-8 gap-sm ">
				<TuringFace className="text-white"/>
			</div>
			<h2 class="text-title outline-none bg-transparent truncate {triggerClassName}">
				{#if selectedModel}
					{selectedModel.label}
				{:else}
					{placeholder}
	...
	<DropdownMenu.Content
		class=" z-40 {$mobile
			? `w-full`
			: `${className}`} justify-start rounded-xl  bg-slate-100 dark:bg-slate-900 dark:text-white shadow-lg  outline-none"
...
<!--		<slot>-->
<!--			<header class="flex items-center justify-between p-lg bg-lvl-2 border-b">-->
<!--				<h1 class="text-title">Cambiar asistentes</h1>-->
<!--				<Button onClick={() => (show = false)} icon={X} class="text-slate-500"/>-->
<!--			</header>-->
<!--			<section class="p-2xl">-->
...
<!--					{#if searchEnabled}-->
<!--						<div class="flex items-center gap-2.5 p-base rounded-sm border border-slate-700 bg-slate-950">-->
<!--							<input-->
<!--								id="model-search-input"-->
<!--								bind:value={searchValue}-->
<!--								class="w-full placeholder:text-placeholder placeholder:text-slate-500 bg-transparent outline-none"-->
...
<!--									&lt;!&ndash; {JSON.stringify(item.info)} &ndash;&gt;-->

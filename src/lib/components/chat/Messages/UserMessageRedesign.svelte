				...
				<div class="w-full">
					<div class="flex {($settings?.chatBubble ?? true) ? 'justify-end pb-1' : 'w-full'}">
						<div
							class="rounded-md {($settings?.chatBubble ?? true)
								? `max-w-[90%] p-base text-slate-900 dark:text-slate-50 bg-brand-100 dark:bg-brand-900`
								: ' w-full'}"
						>
							{#if message.content}
					...
					<div
						class=" flex {($settings?.chatBubble ?? true)
							...
							: ''}  text-slate-500"
					>
						<div class="invisible group-hover:visible flex justify-end bg-slate-50 dark:bg-slate-900 p-xs gap-sm items-center rounded-sm">
							{#if !($settings?.chatBubble ?? true)}
